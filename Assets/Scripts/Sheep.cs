...
namespace Completed
{
	...
	public class Sheep : MovingObject
	{
		...
		//MoveSheep is called by the GameManger each turn to tell each Sheep to try to move towards the player.
		public void MoveSheep ()
		{
			...
			if(Mathf.Abs (target.position.x - transform.position.x) < float.Epsilon)
				...
				//If the y coordinate of the target's (player) position is greater than the y coordinate of this sheep's position set y direction 1 (to move up). If not, set it to -1 (to move down).
				yDir = target.position.y > transform.position.y ? 1 : -1;
			
			//If the difference in positions is not approximately zero (Epsilon) do the following:
			else
				//Check if target x position is greater than sheep's x position, if so set x direction to 1 (move right), if not set to -1 (move left).
